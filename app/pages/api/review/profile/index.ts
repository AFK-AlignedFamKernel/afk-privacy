...
export async function getMyData(
  ...
  try {
    ...
    const { data: passportData, error: passportError } = await supabase
      ...
      .select("id_register, nationality, date_of_birth, gender, is_verified, proof, proof_args")
      .eq("pubkey", signedMessage.ephemeralPubkey.toString())
      // .eq("id_register", signedMessage.ephemeralPubkey.toString())
      .single();


    console.log("passport_data", passportData);
    if (passportError) {
      throw passportError;
    }
