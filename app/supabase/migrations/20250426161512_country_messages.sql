...
CREATE TABLE IF NOT EXISTS country_messages (
    ...
    CONSTRAINT fk_passport_registration
        FOREIGN KEY (country, age_range, gender)
        REFERENCES passport_registrations(country, age_range, gender)
);

CREATE INDEX IF NOT EXISTS idx_country_messages_country ON country_messages(country);
CREATE INDEX IF NOT EXISTS idx_country_messages_age_range ON country_messages(age_range);
