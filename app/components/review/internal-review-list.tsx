...
const InternalReviewList: React.FC<ReviewListProps> = ({
  isInternal,
  groupId,
  ...
  const fetchPolls = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/poll/list-internal?groupId=${groupId}&country=${country}`, {
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${ephemeralPubkey?.toString()}`
        },
        body: JSON.stringify({
          groupId,
          country,
          ...signedMessageFormatted,
          signedMessageFormatted: signedMessageFormatted,
        }),
      });
      if (!response.ok) {
...
export default InternalReviewList;
